...
st.caption("Now with real news sentiment, enhanced BUY/SELL logic, and headline display.")
...
# Real News Sentiment
# ---------------------------------------------------------------
def fetch_real_news_sentiment(ticker: str, api_key: str) -> float:
    ...
    try:
        ...
        scores = [analyzer.polarity_scores(a["title"] + " " + a.get("description", ""))["compound"] for a in articles[:10]]
...
# ---------------------------------------------------------------
# Signal Logic (news sentiment weighted)
# ---------------------------------------------------------------
def generate_signal(ind: pd.DataFrame, sentiment_score: float, mode: str):
    ...
    # News Sentiment (double impact)
    score += float(np.clip(sentiment_score * 2, -1, 1))
...
if ticker:
    ...
    if df is None:
        ...
        sentiment = fetch_real_news_sentiment(ticker, api_key)
        ...
        headlines = fetch_headlines(ticker, api_key)
        if headlines:
