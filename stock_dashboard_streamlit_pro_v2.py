...
st.caption("More responsive BUY/SELL logic, backtest accuracy, trading mode, and 2-year context.")
...
# ---------------------------------------------------------------
# Sentiment (lightweight placeholder – deterministic but variable)
# ---------------------------------------------------------------
def generate_sentiment(ticker: str) -> float:
    analyzer = SentimentIntensityAnalyzer()
    headlines = [
        f"{ticker} outlook improves as analysts raise targets",
        f"{ticker} faces regulatory pressure and margin risks",
        f"Investors weigh {ticker} growth vs. valuation",
    ]
    scores = [analyzer.polarity_scores(h)["compound"] for h in headlines]
    return float(np.mean(scores))

# ---------------------------------------------------------------
# Signal Logic (weighted + mode sensitivity)
# ---------------------------------------------------------------
def generate_signal(ind: pd.DataFrame, sentiment_score: float, mode: str):
    ...
    # Sentiment contributes gently
    score += float(np.clip(sentiment_score, -0.5, 0.5))

    # Thresholds adapt to mode
    buy_th = 2.5 * sens
    sell_th = -1.5 * sens
    ...
    return signal, color, round(score, 2)

# ---------------------------------------------------------------
# Lightweight Backtest (signal preview)
# ---------------------------------------------------------------
def backtest_preview(ind: pd.DataFrame) -> float:
    """Very rough 5-day look-ahead accuracy using RSI extremes."""
    test = ind.copy()
    test["Sig"] = 0
    ...
    # Only evaluate where a signal existed
    mask = test["Sig"] != 0
    if mask.sum() < 5:
        return 0.0
...
def plot_indicators(ind: pd.DataFrame, ticker: str):
    ...
    fig.update_layout(height=820, title=f"{ticker} — Technical Dashboard (2y)",
...
def explain_signal(ind: pd.DataFrame, sentiment: float, name: str) -> str:
    ...
    if last["Close"] < last["BB_Low"]:
        reasons.append("Price under lower Bollinger → potential bounce")
    elif last["Close"] > last["BB_Up"]:
        reasons.append("Price above upper Bollinger → stretched")
    reasons.append("Sentiment mildly positive" if sentiment > 0.1 else ("Sentiment mildly negative" if sentiment < -0.1 else "Neutral sentiment"))
    return f"**Why {name}:** " + ", ".join(reasons)

def confidence_from_score(score: float, signal: str) -> tuple[float, str]:
    conf = float(min(abs(score) / 3.5, 1.0))
    if signal == "BUY":
        text = "High-conviction entry zone" if conf > 0.7 else "Cautious buy — wait for follow-through"
    elif signal == "SELL":
...
if ticker:
    ...
    if df is None:
        ...
        st.write(f"**Backtest preview (RSI swing rule, 5-day horizon):** {acc:.1f}% accuracy")
...
© 2025 Raj Gupta — *AI Stock Signals Dashboard PRO v2*
"""
)
