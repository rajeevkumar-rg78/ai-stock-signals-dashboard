# AI Stock Signals PRO — Business-Ready (v5.5.6, live price fix)
# --------------------------------------------------------------------
# Changes vs your version:
# - Correct live price/delta using Yahoo quote API (with 1m fallback)
# - Keeps your full UI/logic: tutorial, indicators, forecast, sims, macro,
#   analyst pulse, tech screener, news, learn, disclaimer
# --------------------------------------------------------------------
...
def render_header(decision: str = "HOLD"):
    ...
    if "BUY" in decision:
        grad = "linear-gradient(270deg, #43e97b 0%, #38f9d7 100%)"; accent_emoji = "🟢"
    elif "SELL" in decision:
        grad = "linear-gradient(270deg, #fa709a 0%, #fee140 100%)"; accent_emoji = "🔴"
    else:
        grad = "linear-gradient(270deg, #30cfd0 0%, #330867 100%)"; accent_emoji = "🟠"

    st.markdown(
        f"""
        <div style="position:relative;background:{grad};background-size:400% 400%;
            ...
            <div style="position:relative;z-index:1;">
                <div style="display:flex;align-items:center;justify-content:space-between;">
                    <div style="display:flex;align-items:center;gap:18px;">
                        ...
                        <div>
                            <div style="font-size:25px;font-weight:800;">AI Stock Signals PRO</div>
...
# 👋 eye-catching onboarding banner
st.markdown("""
<div style="background:linear-gradient(90deg,#4A00E0 0%,#8E2DE2 100%);color:white;
    text-align:left;padding:14px 18px;border-radius:10px;margin-top:8px;margin-bottom:10px;
    box-shadow:0 4px 10px rgba(0,0,0,0.15);font-size:17px;font-weight:600;">
...
with st.expander("🎓 Learn How AI Stock Signals PRO Works", expanded=False):
    st.markdown("""
AI Stock Signals PRO blends **Technicals, Macro, News Sentiment, Analyst trends, and an AI forecast** to generate an educational buy/hold/sell view with targets and stops. It’s for **education & research** only — no live brokerage execution.
""")
...
cA.metric("Price", f"${live_price:.2f}", delta=f"{delta:+.2f} ({delta_pct:+.2f}%)")
...
# Use live_price for bands; ATR from indicators
target_up = live_price + 2.0*last["ATR"]
buy_zone  = live_price - 1.5*last["ATR"]
stop_loss = live_price - 2.5*last["ATR"]
...
with tab1:
    ...
    day_action = daily_action_strategy(
        live_price, buy_zone, target_up, stop_loss, decision, invest_amount, shares_held=0, cash=invest_amount
    )
...
with tab3:
    ...
    if sim_prices is None:
        ...
        ax.set_xlabel("Price"); ax.set_ylabel("Sims")
        plt.tight_layout()
        st.pyplot(fig)
